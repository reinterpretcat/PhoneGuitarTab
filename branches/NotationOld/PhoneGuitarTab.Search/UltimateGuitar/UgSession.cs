...
using PhoneGuitarTab.Cryptography;

namespace PhoneGuitarTab.Search.UltimateGuitar
{
    /// <summary>
    /// Represents session for requets to ultimate guitar resources
    /// </summary>
    public class UgSession : ISession, IDisposable
    {
        ...
        private UgSession()
        {
            RequstInspector = new UgRequestInspector();
        }
            ...
            get
            {
                lock (__monitor)
                {
                    ...
                    if (_instance == null) 
                    {
                        ...
                        //string name = (string) SearchContext.Instance["name"];
                        //Note: security violation
                        //string pass = (string) SearchContext.Instance["password"];
                       //_instance.LogIn(name, pass);
                    }
                }
                return _instance;
            }
        ...
        private bool AuthExistInSetCookieHeader(HttpWebResponse response)
        {
            ...
            return
                //CanExtractCookie(setCookie, "bblastvisit", response) &&
                //CanExtractCookie(setCookie, "bblastactivity", response) &&
                //CanExtractCookie(setCookie, "backing_tracks", response) &&
                //CanExtractCookie(setCookie, "ads_free", response) &&
               // CanExtractCookie(setCookie, "etp", response) &&
